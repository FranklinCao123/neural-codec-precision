"""Run the BF16 AMP codec and forward benchmark."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        default="configs/cheng2020_bf16.yaml",
        help="Path to BF16 experiment config.",
    )
    parser.add_argument(
        "--device",
        default="auto",
...
def main() -> None:
    ...
    if device.type != "cuda":
        print("Warning: BF16 autocast is only enabled on CUDA; falling back to FP32.")

    model = load_model_from_config(config)
    dataloader = build_image_dataloader(config)

    results = evaluate_codec(model, dataloader, config, device=device)
    output_dir = PROJECT_ROOT / config.get("output", {}).get("dir", "results/raw/cheng2020_bf16")
