"""Run fixed-point activation probe experiments."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        default="configs/cheng2020_int16_fixed_probe.yaml",
        help="Path to fixed-point probe config.",
    )
    parser.add_argument(
        "--device",
        default="auto",
...
def main() -> None:
    ...
    model = apply_fixed_point_probe(model, config)
    dataloader = build_image_dataloader(config)

    results = evaluate_codec(model, dataloader, config, device=device)
    output_dir = PROJECT_ROOT / config.get("output", {}).get("dir", "results/raw/fixed_point_probe")
    save_results(results, output_dir)
    save_fixed_point_probe_report(model, output_dir)

    summary = results["summary"]
    print(f"Saved results to: {output_dir}")
    print(f"Saved fixed-point probe to: {output_dir / 'fixed_point_probe.json'}")
    print(
        "Summary: "
