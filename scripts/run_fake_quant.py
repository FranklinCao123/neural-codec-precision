"""Run activation fake-quantization precision experiments."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Path to fake-quant experiment config.",
    )
    parser.add_argument(
        "--device",
        default="auto",
...
def main() -> None:
    ...
    model = apply_activation_fake_quant(model, config)
    dataloader = build_image_dataloader(config)

    results = evaluate_codec(model, dataloader, config, device=device)
    output_dir = PROJECT_ROOT / config.get("output", {}).get("dir", "results/raw/fake_quant")
